...
import { EMPTY_BODY, body, paragraph } from './body';
...
describe(mentionsOf, () => {
  it('finds notes that link by title', () => {
    const linker = note({ id: 'a', body: prose('см. [[Планы на неделю]]') });
    const other = note({ id: 'b', body: prose('ничего общего') });
  ...
  it('case and extra spaces in the link do not interfere', () => {
    const linker = note({ id: 'a', body: prose('[[планы   НА неделю]]') });
    expect(mentionsOf(target, [target, linker])).toHaveLength(1);
  });

  it('link with a label counts', () => {
    const linker = note({ id: 'a', body: prose('[[Планы на неделю|туда]]') });
    expect(mentionsOf(target, [target, linker])).toHaveLength(1);
  });

  it('note does not mention itself', () => {
    const selfish = note({ id: 'target', title: 'Планы на неделю', body: prose('[[Планы на неделю]]') });
  ...
  it('untitled note has no mentions: there is nothing to link to', () => {
    ...
    const linker = note({ id: 'a', body: prose('[[]]') });
    expect(mentionsOf(empty, [empty, linker])).toEqual([]);
  });

  it('archived note does not count as a mention: it was hidden from view', () => {
    const linker = note({ id: 'a', body: prose('[[Планы на неделю]]'), archived: true });
    expect(mentionsOf(target, [target, linker])).toEqual([]);
  });

  it('mention as plain text without brackets does not count', () => {
    const linker = note({ id: 'a', body: prose('обсудили планы на неделю') });
    expect(mentionsOf(target, [target, linker])).toEqual([]);
  });

  it('a link inside any block counts, not only inside a paragraph', () => {
    const linker = note({ id: 'a', body: body(paragraph('план'), paragraph('см. [[Планы на неделю]]')) });
    expect(mentionsOf(target, [target, linker])).toHaveLength(1);
  });
  ...
  it('mentions follow the list order: pinned first, then by freshness', () => {
    const all = [
      target,
      note({ id: 'a', body: prose('[[Планы на неделю]]'), updatedAt: 10 }),
      note({ id: 'b', body: prose('[[планы на неделю]]'), updatedAt: 30 }),
      note({ id: 'c', body: prose('[[Планы на неделю]]'), updatedAt: 1, pinned: true }),
    ];
