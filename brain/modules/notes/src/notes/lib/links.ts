 ...
 *
 * На вход приходит текст блоков без разметки (`bodyText` из `entities/body.ts`),
 * поэтому отличить ссылку от `[[…]]` внутри кодового блока этот слой не может.
 * Когда ссылка станет маркой документа редактора, догадки уйдут вместе с этим
 * файлом.
 */
