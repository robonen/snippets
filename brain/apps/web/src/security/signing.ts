...
export function makeSecure(
  ...
  const dropSealed = (error: CryptoError): void => {
    ...
    complained.add(at);
    // Ожидаемо, не поломка: у члена пространства это безвредный след прежних
    // секретов на сервере, а устройству вне пространства экран «Доступ» и так
    // объясняет его положение. Одна строка на ленд.
    console.warn(
      `[brain] синк: часть юнитов (${at}) запечатана недоступным секретом и пропущена — `
      + 'след старых данных на сервере; новому устройству нужно приглашение.',
    );
  };
