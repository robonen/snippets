...
export default defineConfig({
  ...
  runtimeConfig: {
    ...
    syncToken: '',
    /**
     * Origin для СТРОГОЙ сверки заголовка `Origin` на WS-рукопожатии.
     * Пусто (по умолчанию) — сверка выключена: авторизация держится на токене
     * в query, а его, в отличие от cookie, чужой сайт браузеру не подсунет —
     * cross-site hijacking без ambient-креденшала не работает. Задан явно —
     * рукопожатия с других origin отвергаются.
     */
    publicOrigin: '',
