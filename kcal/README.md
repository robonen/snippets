...
- **Redis pub/sub между инстансами** — синхронизация устройств работает живьём
  (WebSocket с бинарными кадрами, POST как запасной путь; включается
  `SYNC_TOKEN`/`VITE_SYNC_TOKEN`), правка доезжает за ~50 мс. Но вещание пока в
  пределах одного инстанса: между инстансами Vercel дельту добирают приветы по
  расписанию — см. [docs/server-sync.md](docs/server-sync.md).
