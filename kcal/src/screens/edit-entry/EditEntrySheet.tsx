...
import { useActions, useEntries } from '@/db/composables';
 ...
 */
export default function EditEntrySheet(props: {
  entryId: string;
  onClose: () => void;
}) {
  const entries = useEntries();
  const entry = computed(() => entries.value.find(item => item.id === props.entryId));
  ...
  const removeEntry = () => {
    const current = entry.value;
    if (!current) return;
    actions.removeEntry(current.id);
    close();
  };
