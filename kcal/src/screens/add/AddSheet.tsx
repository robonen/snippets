...
import { useSheetDrag } from '@/shared/lib/sheetDrag';
...
import { fetchOffByBarcode } from '@/features/barcode/off';
...
import { IconBarcode, IconChevronLeft, IconClose, IconSearch } from '@/shared/ui/icons';
import BarcodeScanner, { isBarcodeScanSupported } from '@/features/barcode/BarcodeScanner';
...
export default function AddSheet(props: {
  /** День дневника, в который уйдёт запись. */
  date: string;
  /** Приём пищи, из которого открыли шторку (внутри можно переключить). */
  meal: Meal;
  onClose: () => void;
}) {
  const step = shallowRef<Step>('pick');
  const query = shallowRef('');
  const meal = shallowRef<Meal>(props.meal);
  ...
  return (
    <div class="fixed inset-0 z-50 flex items-end justify-center">
      <div ref={drag.overlay} class="sheet-overlay" onClick={close} />

      <div
        ref={drag.panel}
        class="sheet-panel max-h-[90dvh]"
