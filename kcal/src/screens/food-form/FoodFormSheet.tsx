...
import { IconClose, IconTrash } from '@/shared/ui/icons';

/** Создание и правка продукта каталога. Значения нутриентов — на 100 г. */
export default function FoodFormSheet(props: {
  /** Продукт на правке; null — создание нового. */
  foodId: string | null;
  onClose: () => void;
}) {
  const foods = useFoods();
  const existing = computed(() => foods.value.find(food => food.id === props.foodId));
