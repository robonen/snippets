...
import { dayTitle, shiftISODate, todayISO } from '@/shared/lib/dates';
...
import { IconChevronLeft, IconChevronRight, IconPlus } from '@/shared/ui/icons';
import ProgressRing from './ProgressRing';
import MacroBar from './MacroBar';

export default function DiaryScreen(props: {
  /** Открытый день дневника (YYYY-MM-DD); записи добавляются именно в него. */
  date: string;
  ...
  const entries = computed(() => all.value.filter(entry => entry.date === props.date));
    ...
    <div class="flex flex-col gap-5 pb-6">
      ...
      <div class="animate-rise flex items-center justify-between">
        <button
          ...
          onClick={() => props.onDateChange(shiftISODate(props.date, -1))}
        >
          <IconChevronLeft />
        </button>
        <div class="text-center">
          <div class="text-display text-xl font-medium">{dayTitle(props.date)}</div>
          {!isToday.value && (
            <button type="button" class="mt-0.5 text-xs text-ember-bright/90 hover:text-ember-bright" onClick={() => props.onDateChange(todayISO())}>
              вернуться к сегодня
            </button>
          )}
        </div>
