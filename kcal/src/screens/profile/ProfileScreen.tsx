...
import { useActions, useProfile } from '@/db/composables';
import { useSpace } from '@sync/vue';
import { downloadBackupFile, exportBackup, importBackup } from '@/features/backup';
import type { BackupPayload } from '@/features/backup';
import { ACTIVITY_LEVELS, GOAL_LABELS, bmr, computeTargets, safeKcalFloor, tdee } from '@/entities/profile';
import { roundTo } from '@/shared/lib/numbers';
import { fmtKcal } from '@/shared/lib/format';
import type { Goal, Profile, Sex } from '@/entities/profile';
...
export default function ProfileScreen(props: { onboarding?: boolean }) {
  ...
  const doExport = async () => {
    downloadBackupFile(exportBackup(space));
  };
  ...
  const wipeAll = () => {
    ...
    // Новая база — данные; старая (`kcal`) больше не читается вовсе, но у
    // прежних установок она ещё занимает место — сносим заодно.
    indexedDB.deleteDatabase('kcal-sync');
    indexedDB.deleteDatabase('kcal');
    location.reload();
  };
