...
Синхронизация разложена по `src/db/sync/`: `protocol.ts` — общая для обоих
транспортов логика «принять пачку, при фейсах ответить встречной дельтой»;
`http.ts` — POST; `socket.ts` — WebSocket; `index.ts` — оркестратор.

Живой путь — сокет, POST остаётся страховкой. Правила, которые стоили замеров:

- **кадры бинарные в обе стороны** — в сокет уходит тот же `Uint8Array`, что и в
  тело POST, обратно `ArrayBuffer` (`binaryType`). Ни base64, ни JSON: пачка и
  так компактный бинарь, текстовая обёртка раздула бы её примерно на треть;
- **первый привет ждёт сокет ~1 с.** Без паузы старт стоил двух полных дельт —
  HTTP-приветом и следом кадром (на живом ленде это были лишние 170 КБ). Сокет
  поднимается за десятки миллисекунд, а если не поднялся — уходит запасной
  HTTP-привет;
- **пока сокет жив, таймерные приветы молчат** — он уже принёс всё;
- **привет на КАЖДОМ коннекте** закрывает пропуски за обрыв: на Vercel
  соединение не переживает `maxDuration`, а телефон рвёт его при блокировке
  экрана. Реконнект по отступам 1 → 2 → 5 → 10 → 30 с.
...
| 7 | клиентский живой транспорт в `src/db/sync/socket.ts`: бинарные кадры, привет на каждом коннекте, реконнект с отступом, HTTP как запасной путь | ✅ 9 тестов против настоящего `exchange` с вещанием; замер в браузере — 49 мс от нажатия до появления на втором устройстве |
