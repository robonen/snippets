...
describe('чего ленд НЕ делает (записанные дыры, не поблажки)', () => {
  /**
   * Ленд принимает только санды. Виды `gift`/`seal`/`pass` он отбрасывает
   * НАСОВСЕМ: их нет ни в арене (`apply`), ни в выдаче (`units`), поэтому
   * реплика, через которую пачка прошла транзитом, пересылает дальше пачку без
   * прав и подписей. Пропуск объявлен работой S6/S7, но пропуск на ПРИЁМЕ — это
   * потеря, а не отложенная функциональность.
   */
  test.fails('круг через ленд сохраняет все четыре вида юнитов', () => {
    const units: AnyUnit[] = [
      SandUnit.make({
